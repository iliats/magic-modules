...
func testAccComputeStoragePoolTagBindingCheckConfig(t *testing.T, resourceName string, expectedTagValueResources, unexpectedTagValueResources []string) acctest.TagBindingCheckConfig {
	return acctest.TagBindingCheckConfig{
		...
		BuildParent: func(rs *terraform.ResourceState) (string, error) {
			...
			poolID, ok := res["id"].(string)
			if !ok || poolID == "" {
				return "", fmt.Errorf("expected id to be set for %s. got id=%q", resourceName, res["id"])

			}

			return fmt.Sprintf("//compute.googleapis.com/projects/%s/zones/%s/storagePools/%s", project, zone, poolID), nil
		},
